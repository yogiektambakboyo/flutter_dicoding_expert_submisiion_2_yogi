...
class Shared {
  static Future<HttpClient> customHttpClient({
    ...
    try {
      ...
      if (isTest == true) {
        ...
        bytes = (await rootBundle.load('assets/ssl_cert.pem')).buffer.asUint8List();
      }
  ...
  static Future<http.Client> createLEClient({bool isTest = false}) async {
    IOClient client = IOClient(await Shared.customHttpClient(isTest : isTest));
    return client;
  }
}
